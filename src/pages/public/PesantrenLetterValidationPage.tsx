...
export default function PesantrenLetterValidationPage() {
  const { id } = useParams();
  ...
  useEffect(() => {
    async function loadValidation() {
      ...
      const { data, error: rpcError } = await supabase.rpc("validate_pp_surat_keluar", {
        p_id: id,
      });
      setLoading(false);
      if (rpcError) {
  ...
  useEffect(() => {
    ...
    async function loadDocument() {
      ...
      const { data } = await supabase.storage.from("surat-keluar").download(fileUrl);
      if (!data) return;
        ...
        {loading ? (
            ...
            <div className="flex items-start gap-3 text-red-700">
              ...
              <div>
                ...
                <p className="mt-2 text-sm text-gray-600">
                  {error || "Kode QR ini tidak cocok dengan arsip surat pesantren."}
                </p>
              </div>
            </div>
          </div>
          ...
          <>
            ...
            <div className="grid gap-5 md:grid-cols-2">
              ...
              <div className="rounded bg-white p-6 shadow-soft">
                <h3 className="font-semibold text-gray-950">Data Santri Terkait</h3>
                <div className="mt-4 grid gap-3 text-sm">
