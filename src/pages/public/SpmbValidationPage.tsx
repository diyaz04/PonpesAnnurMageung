...
export default function SpmbValidationPage() {
  const { id } = useParams();
  ...
  useEffect(() => {
    async function loadValidation() {
      ...
      const { data, error: rpcError } = await supabase.rpc("validate_smp_spmb", {
        p_id: id,
      });
      setLoading(false);
      if (rpcError) {
  ...
  const proofUrl = payload?.pendaftar?.bukti_url
    ? supabase.storage.from("smp-spmb-bukti").getPublicUrl(payload.pendaftar.bukti_url).data.publicUrl
    : "";
  const photoUrl = payload?.pendaftar?.foto_url
    ? supabase.storage.from("smp-spmb-foto").getPublicUrl(payload.pendaftar.foto_url).data.publicUrl
    : "";
            ...
            <div className="flex items-start gap-3 text-red-700">
              ...
              <div>
                ...
                <p className="mt-2 text-sm text-gray-600">
                  {error || "Kode QR ini tidak cocok dengan arsip SPMB sekolah."}
                </p>
              </div>
            </div>
          </div>
            ...
            <div className="rounded bg-white p-6 shadow-soft">
              <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <h2 className="text-xl font-semibold text-gray-950">Bukti SPMB Valid</h2>
                    <p className="mt-1 text-sm text-gray-600">
                      Data pendaftaran cocok dengan arsip resmi sekolah.
                    </p>
                  </div>
                </div>
                {proofUrl ? (
              ...
              <div className="rounded bg-white p-6 shadow-soft">
                ...
                <div className="mt-4 grid gap-4 sm:grid-cols-[112px_1fr]">
                  ...
                  <div className="grid gap-3 text-sm">
                    ...
                    <p><span className="text-gray-500">Asal Sekolah:</span> {payload.pendaftar?.asal_sekolah || "-"}</p>
